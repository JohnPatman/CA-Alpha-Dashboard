...
def dark_table(rows, headers, highlights=None, height=None):
    ...
    html = (
        ...
        'var ind=th.querySelector(".si");if(ind)ind.textContent=asc?" \u25b2":" \u25bc";'
        'rs.forEach(function(r){tb.appendChild(r);});}'
        '</script>'
        '</head><body>'
        f'<table><thead><tr>{th}</tr></thead><tbody>{tbody}</tbody></table>'
        '<script>Array.from(document.querySelectorAll("thead th")).forEach(function(th){th.onclick=function(){sT(th);};});</script>'
        '</body></html>'
    )
